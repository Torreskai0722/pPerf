# Fix for imgaug compatibility with newer NumPy versions
import numpy as np
...
def aug_image_rain(rain_rate, image_file, output_file):
    ...
    # Set deterministic seed based only on rain rate
    # This ensures all images get the same rain pattern for the same rain rate
    seed = rain_rate  # Simple seed based on rain rate
    np.random.seed(seed)
