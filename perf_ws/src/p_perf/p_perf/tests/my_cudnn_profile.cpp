...
static BackendDesc build_conv_op_graph(
    ...
    TORCH_CHECK(groups == 1,
                "Backend profiling path: groups>1 not supported by this cuDNN backend header/version.");
...
profile_conv2d_variants(
    ...
    for (int64_t i = 0; i < useCfgs; ++i) {
        ...
        checkCudnnBackend(cudnnBackendFinalize(variantPack), "finalize variantPack");
...
TORCH_LIBRARY(my_cudnn_prof, m) {
    m.def("profile_conv2d(Tensor input, Tensor weight, Tensor? bias, int[] stride, int[] padding, int[] dilation, int groups, int max_engine_variants, int iters) -> (Tensor, Tensor)");
}

TORCH_LIBRARY_IMPL(my_cudnn_prof, CUDA, m) {
