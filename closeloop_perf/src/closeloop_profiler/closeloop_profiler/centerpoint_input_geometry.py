...
def sha256(path):
    """Hash retained input evidence without loading whole bags into memory."""
...
def reconstruct(root):
    ...
    for scene in dict.fromkeys(r["scene_id"] for r in timings):
        ...
        source_frames = [json.loads(line) for line in Path(manifest["frame_index"]).read_text().splitlines() if line]
        index = {r["output_header_timestamp_ns"]: r for r in source_frames
