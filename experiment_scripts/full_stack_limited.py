...
lidar_queues = [1]
publishing_rate = [10]
seg_mps_percentages = [20, 30, 40, 50, 60, 70, 80, 90, 100]  # MPS percentages for segmentation models
output_name = "full_stack_1I1L1S-MPS-limited-DETR"
...
combinations = list(product(depths, image_models, lidar_models, seg_models, scenes, image_queues, lidar_queues, publishing_rate, seg_mps_percentages))
...
if OVERWRITE:   
    with open(mapping_file, mode='w', newline='') as csvfile:
        ...
        writer.writerow(["run_index", "scene", "depth", "image_model", "lidar_model", "seg_model", "image_queue", "lidar_queue", "publishing_rate", "seg_mps_percentage", "status", "start_time"])
        for i, (depth, img_model, lidar_model, seg_model, scene, img_q, lidar_q, publishing_rate, seg_mps) in enumerate(combinations):
            writer.writerow([i, scene, depth, img_model, lidar_model, seg_model, img_q, lidar_q, publishing_rate, seg_mps, "pending", ""])
