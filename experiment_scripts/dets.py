...
if OVERWRITE and not CONTINUE:   
    ...
    for i, (depth, img_model, lidar_model, scene, img_q, lidar_q, publishing_rate) in enumerate(combinations):
        ...
        if not combo_exists:
            new_combinations.append([start_index + len(new_combinations), scene, depth, img_model, lidar_model, img_q, lidar_q, publishing_rate, "pending", ""])
...
for i, row in df.iterrows():
    ...
    layer_timings = f'{output_base}/layer_timings_{i}.csv'
    if os.path.exists(layer_timings):
        continue
